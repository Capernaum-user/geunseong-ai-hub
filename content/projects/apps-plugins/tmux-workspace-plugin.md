...
> **한마디로:** 노트 앱의 **버튼 하나로 WSL 안의 tmux 작업 세션을 통째로** 띄우는 플러그인입니다. 매번 손으로 터미널 열고 세션·창을 배치하던 과정을 한 번에 처리합니다.

실행 전 검사(preflight)와 터미널 폴백, 디버그 로그로 "조용히 실패하지 않게" 만든 것이 핵심입니다.

```mermaid
flowchart LR
    R[리본 버튼] --> P[preflight 검사]
    P --> T{Windows Terminal?}
    T -->|성공| W[tmux 세션 구성]
    T -->|실패| F[PowerShell 폴백] --> W
```
