...
> **한마디로:** 하네스는 흩어진 파일 모음이 아니라, 세 AI(Claude·Gemini·Codex)가 볼트를 **일관되게 운영하도록 묶은 규칙집(운영체제)** 입니다.

새 세션은 항상 같은 순서로 규칙을 읽고 시작합니다 — 헌법 → 미션 → 작업 실행 → 완료 표준 → 변경 이력.

```mermaid
flowchart LR
    C[헌법] --> M[미션·맥락] --> E[작업 실행] --> D[완료 표준] --> H[변경 이력]
```
