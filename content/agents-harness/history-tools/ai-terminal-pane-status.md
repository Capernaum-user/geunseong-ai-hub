...
> [!abstract] 한 줄 요약
> 터미널을 여러 칸으로 쪼개 AI CLI(claude·codex·gemini)를 동시에 돌리다 보면 **"이 칸이 무슨 작업이었지?"** 하고 길을 잃는다. 이 도구는 각 패널 하단에 **폴더·git브랜치·요청작업명 + 대기/확인 색점**을 붙이고, 모든 세션의 **🟡작업중 / 🟢대기 / 🔴확인** 을 실시간 상태창으로 보여준다. 순수 Windows Terminal에서 동작하고, **단일 파일로 원클릭 설치**된다. 소스 공개: [GitHub](https://github.com/Capernaum-user/ai-terminal-pane-status).
...
🔴 📁 D:\01_Projects\03_Guesthouse · 🌿 main · Opus 4.8 · +3/-0 · 12:04 · ctx 88% ⚠ /compact 권장 · 스키마 리서치
5h [████░░░░░░] 38% ↻2h13m (2026/07/02 PM 10:00)   7d [█████████░] 86% ↻4d21h (2026/07/05 AM 09:30)
```

- **1줄** — **맨 앞 상태 색점**(🟢 대기중 · 🔴 확인요청, 실시간) · 실행 폴더 절대경로 · git 브랜치 · 모델 · 추가/삭제 라인 · 경과 시간 · **컨텍스트 사용률**(80%↑ `⚠ /compact` 경고) · **요청 작업명(최대 12자)**
- **2줄(Pro/Max)** — 5시간·7일 사용량을 **배터리 칸**으로, % 와 **초기화까지 남은 시간 + 정확한 초기화 시각(한국시간)**

> [!note] 상태 색점은 왜 대기·확인만 실시간인가
> 상태줄은 **Claude가 멈추는 순간에만** 다시 그려진다. 그 순간이 곧 "답을 끝냄(→🟢 대기)"과 "권한 확인창(→🔴 확인)"이라 이 둘은 **실시간**으로 색이 바뀐다. **작업중(생성 중)**엔 상태줄이 안 그려져 회색 — 작업중까지 실시간으로 보려면 아래 `aistatus` 상태창을 쓴다.
...
| **사용량 배터리** | Pro/Max: 5시간·7일 한도를 칸 막대 + 남은 시간 + 정확한 초기화 시각(KST, `2026/07/02 PM 10:00`) |
