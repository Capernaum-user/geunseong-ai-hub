...
> **한마디로:** **위키가 원천(DB), Quartz가 화면(렌더러)** 입니다. `visibility/public` 태그가 붙은 노트만 한 방향으로 사이트에 발행되고, 빌드할 때 한 번 동기화됩니다.

Hugo 때와 달리 옵시디언 마크다운을 거의 그대로 읽어서, `[[위키링크]]`와 그래프가 그대로 살아납니다.
...
- [[ai-plugins|Obsidian + 생성형 AI 플러그인]]
