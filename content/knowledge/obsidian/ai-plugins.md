...
Obsidian은 그 자체로는 마크다운 에디터지만, 플러그인 몇 개를 얹으면 **AI가 읽고 쓰고 연결하는 지식 작업대**가 된다. 여기서는 [[obsidian-vault|Vault]]를 생성형 AI와 함께 쓸 때 실제로 쓰는 5종을 역할별로 정리한다.

## 쉽게 말하면

> **한마디로:** 옵시디언은 그냥 메모장이지만, **플러그인 5종**을 얹으면 AI가 읽고 쓰고 연결하는 작업대가 됩니다.

각자 역할이 다릅니다 — 챗(Copilot), 자동 생성(Templater), 연결 추천(Smart Connections), 외부 에이전트 입구(Local REST API), 그림(Excalidraw).

```mermaid
flowchart TD
    V[Obsidian Vault] --> C[Copilot<br/>금고 안 챗]
    V --> T[Templater<br/>노트 자동생성]
    V --> S[Smart Connections<br/>의미 연결 추천]
    V --> R[Local REST API<br/>외부 에이전트 입구]
    V --> E[Excalidraw<br/>다이어그램]
```
...
노트 생성 시 프론트매터(`created/updated/tags`)와 본문 골격을 자동으로 채운다. "AI 발행 노트 템플릿"을 만들어 두면 `visibility/public`, `draft: false` 같은 [[mywiki-to-quartz-pipeline|발행 게이트]] 필드를 빠뜨리지 않게 강제할 수 있다. AI가 만든 노트의 형식 일관성을 보장하는 1차 방어선.
...
- **로컬 우선** — 임베딩·RAG는 가능한 한 로컬에서 처리해 데이터 유출면을 줄인다. (참고: [[knowledge/local-llm/index|로컬 LLM]])
- **시크릿 분리** — API key/토큰/세션 파일은 백업·발행 대상에서 제외한다.
- **단방향 발행** — 플러그인으로 만든 노트도 [[mywiki-to-quartz-pipeline|발행 파이프라인]]의 `visibility/public` 게이트를 통과해야만 사이트로 나간다.
